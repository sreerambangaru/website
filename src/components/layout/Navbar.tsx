    ...
    >
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2">
          <div className="h-10 w-10 rounded-full bg-button-gradient flex items-center justify-center shadow-lg">
            <span className="text-white font-display font-bold text-xl">G</span>
          </div>
          <span className={`font-display font-bold text-xl ${isScrolled ? "text-groq-dark-blue" : "text-white"}`}>
            Groqify
          </span>
        </Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-lg shadow-lg py-6 px-4 absolute w-full">
          <nav className="flex flex-col gap-4">
            ...
            <Button className="bg-button-gradient w-full">
              Join Waitlist
            </Button>
          </nav>
        </div>
...
const NavLinks = ({ isScrolled = true, isMobile = false }: { isScrolled?: boolean; isMobile?: boolean }) => {
  const linkStyle = isScrolled || isMobile
